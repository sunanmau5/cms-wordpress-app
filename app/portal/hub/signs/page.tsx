...
function rr(x: number, y: number, w: number, h: number, r: number) {
  return `M${x + r} ${y} H${x + w - r} A${r} ${r} 0 0 1 ${x + w} ${y + r} V${
    y + h - r
  } A${r} ${r} 0 0 1 ${x + w - r} ${y + h} H${x + r} A${r} ${r} 0 0 1 ${x} ${
    y + h - r
...
const SIGNS: Sign[] = [
  ...
  {
    n: 2,
    note: "red · white bulbs · scalloped",
    bg: RED,
    border: CREAM,
    bulb: CREAM,
    text: "#ffffff",
  ...
  {
    n: 3,
    note: "deep red · alternating gold/white bulbs",
    bg: DEEPRED,
    border: GOLD,
    bulb: GOLD,
    bulb2: "#fff",
  ...
  {
    n: 4,
    note: "red · navy trim · white bulbs",
    bg: RED,
    border: NAVY,
    bulb: "#fff",
    text: "#ffffff",
...
export default function SignChooser() {
  return (
    ...
    >
      ...
      >
        Red background, white type, light bulbs, one line. Bulbs animate
        (chase).
      </p>
      <div
        style={{ display: "grid", gap: 22, maxWidth: 620, margin: "0 auto" }}
      >
        {SIGNS.map((s) => (
          <div key={s.n}>
            <div
